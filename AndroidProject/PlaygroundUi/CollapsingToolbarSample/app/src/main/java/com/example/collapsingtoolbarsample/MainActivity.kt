package com.example.collapsingtoolbarsample
...
import com.example.collapsingtoolbarsample.ui.theme.CollapsingToolbarSampleTheme
...
// Let's make a collapsing toolbar for LazyColumn
                val toolbarHeight = 48.dp // 消し去りたい高さ
                val toolbarHeightPx =
                    with(LocalDensity.current) { toolbarHeight.roundToPx().toFloat() }
// our offset to collapse toolbar
                val toolbarOffsetHeightPx = remember { mutableStateOf(0f) } // 比較用の値
...
// happening inside child LazyColumn
                val nestedScrollConnection = remember {
                    object : NestedScrollConnection {
                        override fun onPreScroll(
                            ...
                            Log.d("TAGTAG", "onPreScroll: ${available.y}")
                            /*
                            下から上は - y
                            上から下は + y
                             */
                            ...
                            Log.d("TAGTAGTAG", "onPreScroll: ${newOffset}")

                            toolbarOffsetHeightPx.value = newOffset.coerceIn(-toolbarHeightPx, 0f) // 消す・見えるの上限までOffsetを変える
                ...
                Box(
                    ...
                    Column() {
                        Spacer(modifier = Modifier.height(240.dp).fillMaxWidth().background(Color.Cyan))
